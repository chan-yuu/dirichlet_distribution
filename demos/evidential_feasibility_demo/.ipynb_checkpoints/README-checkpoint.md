...
### 任务本体
- 输入：x（每个样本 2 维特征）
- 输出目标：y_safe ∈ {0,1}（激进动作是否安全）

### Softmax-CE：输入/输出

训练时：

- 输入：x_train, y_train(one-hot)
- 损失：标准 CE（+L2）
...
- 直接输出字典：
  ...
  - p_safe = probs[:,1]
  - entropy
  - uncertainty（这里等于 entropy）
  - risk_score = (1-p_safe) + 0.45*entropy

———

### Evidential-EDL：输入/输出

训练时：

- 输入：x_train, y_train(one-hot)
- 损失：EDL（mse + var + reg*KL + ce_aux + L2）
- 额外：还会喂一批 x_proxy_ood 做 proxy penalty（鼓励 OOD 低证据）

推理时（predict_edl）：

- 输入：x
- 网络先给 logits -> evidence -> alpha=evidence+1
- 输出字典：
  - probs = p（shape [N,2]，p = alpha/alpha0）
  - alpha0（每个样本总证据强度）
  - p_safe = p[:,1]
  - entropy
  - uncertainty = entropy + 2/alpha0
  - risk_score = (1-p_safe) + 0.45*entropy + 0.75*(2/alpha0)
...
1. 在验证集按安全预算选阈值 tau
2. 测试时 risk_score <= tau 就走 aggressive，否则 conservative
3. 统计 collision/progress 等策略指标

计算示例：

给定
- risk_score = [0.10, 0.35, 0.90]
- tau = 0.40
- y_safe = [1, 0, 1]

第2步动作：

- 0.10<=0.40 -> aggressive
- 0.35<=0.40 -> aggressive
- 0.90>0.40 -> conservative

所以 choose_aggressive=[T,T,F]

第3步单样本结果：

- 样本1（agg, y_safe=1）：col=0, prog=1.0
- 样本2（agg, y_safe=0）：col=1, prog=0.05
- 样本3（cons）：col=0.02, prog=0.72

第4步指标：

- Collision Rate = (0 + 1 + 0.02)/3 = 0.34
- Progress = (1.0 + 0.05 + 0.72)/3 = 0.59
