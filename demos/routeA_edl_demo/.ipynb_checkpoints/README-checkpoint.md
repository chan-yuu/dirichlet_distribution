...
数据分析
- 输入 x：二维特征（shape [N,2]）
- 标签 y：4 类标签（训练里转 one-hot）
- 数据划分：train/val/test + ood（OOD 单独生成）

模型推理输出
- ce/ce_ls 推理输出：probs（4 类 softmax 概率），见 dirichlet_distribution/
demos/routeA_edl_demo/edl_routeA_demo.py:156
- edl 推理输出：p_mean（4 类概率）+ alpha0（总证据强度），见
dirichlet_distribution/demos/routeA_edl_demo/edl_routeA_demo.py:163
...
- 如果真实类概率是 0.9：-log(0.9)=0.105（小，说明模型对且有把握）
- 如果是真实类概率 0.1：-log(0.1)=2.303（大，说明模型很不靠谱）
- 如果模型“自信但错了”，NLL 会非常大（惩罚重）
...
ECE 衡量“置信度和真实正确率是否匹配”。

例子：

- 模型说“我 80% 把握”
- 那这批样本真实正确率最好也接近 80%
- 若实际只有 50%，就叫“过度自信”，校准差

ECE 就是在多个置信度区间里统计这种差距，然后加权平均。

所以：

- ECE 小 = 模型“说话算数”（置信度可信）
- ECE 大 = 模型嘴硬/嘴软（置信度不可信）
=> 越小越好
...
ce/ce_ls 只输出 probs，基于 probs + 真值标签来计算指标：

- Accuracy：argmax(probs) 跟 y_true 比
- NLL：看真实类概率 -log probs[y_true]
- Brier：probs 和 one-hot 标签做平方误差
- ECE：按置信度分箱，对比“平均置信度 vs 实际正确率”
- OOD-AUROC：先把 probs 变成 OOD 分数（这里用 entropy），再和 ID/OOD 标签算
AUROC

EDL 在评估时：
1. 先得到 p_mean 和 alpha0
2. 用 p_mean 去算 Accuracy/NLL/Brier/ECE（和 CE 一样的公式）
3. OOD 分数用
 score = entropy(p_mean) + K/alpha0
4. 用 ID/OOD 标签算 OOD-AUROC
其中，网络都是先输出到logits：
1. 网络输出 logits
2. evidence = softplus(logits)
3. alpha = evidence + 1
4. alpha0 = sum(alpha_i)
- alpha0 大：总体证据强，不确定性低
- alpha0 小：证据弱，不确定性高

```mermaid
flowchart TD
    A[输入样本 x] --> B[MLP前向: logits]

    B --> C1[CE / CE_LS分支]
    B --> C2[EDL分支]

    C1 --> D1["probs = softmax(logits)"]
    C2 --> D2["evidence = softplus(logits)"]
    D2 --> D3["alpha = evidence + 1"]
    D3 --> D4["alpha0 = sum(alpha)"]
    D3 --> D5["p_mean = alpha / alpha0"]
    ...
    %% 任务3: OOD检测
    D1 --> G1["CE/CE_LS: score = entropy(probs)"]
    D5 --> G2["EDL: score = entropy(p_mean) + K/alpha0"]
    G1 --> H[ID+OOD分数拼接]
    G2 --> H
    H --> I[OOD-AUROC]
    ...
    %% 训练差异提示
    C1 --> T1[训练损失: CE]
    C1 --> T2[CE_LS: CE + label smoothing]
    C2 --> T3[EDL损失: mse + var + reg*KL + ce_aux]
```
